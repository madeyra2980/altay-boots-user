    ...
    try {
      ...
      try {
        ...
        if (data && (data.orderId || data.order_id)) {
          setLastOrderId(data.orderId || data.order_id)
        }
      } catch (e) {
        console.error('Failed to parse order success data', e)
      }
          ...
          <div>
            {!items.length ? (
              orderMessage === 'SUCCESS' ? (
                <div className="text-center py-20 bg-white rounded-2xl shadow-sm border border-stone-200 px-4">
